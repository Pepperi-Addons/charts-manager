    ...
    */
    class MyChart {
        ...
        update() {
			...
			if (this.data.groups.length > 0)	{  // multiple group by values - show them in the x-axis
			...
			} else { // no group by, show the series in the x-axis
				var datasetData = [];
				var datasetBGColors = [];
				var datasetBorderColor = [];
				for (let i=0; i<this.data.series.length; i++) {
					datasetData.push(this.data.values[0][data.series[i]]);
					let color =  this.getRandomColor();
